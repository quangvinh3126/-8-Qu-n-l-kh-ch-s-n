...
class HotelManagementApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.tab_room_management = ttk.Frame(self.notebook)
        self.tab_customer_service = ttk.Frame(self.notebook)
        ...
        self.notebook.add(self.tab_room_management, text="Quản lý phòng")
        self.notebook.add(self.tab_customer_service, text="Khách hàng & Dịch vụ")
        self.notebook.add(self.tab_operation, text="Nghiệp vụ lưu trú")
        self.notebook.add(self.tab_revenue, text="Thống kê")

        self._build_dashboard_tab()
        self._build_room_management_tab()
        self._build_customer_service_tab()
    ...
    def _build_dashboard_tab(self):
        ...
        help_box = tk.LabelFrame(self.tab_dashboard, text="Luồng demo thuyết trình")
        help_box.grid(row=2, column=0, columnspan=4, sticky="nsew", padx=10, pady=10)
        help_text = (
            "1. Vào Quản lý phòng để khai báo loại phòng, tạo phòng và chỉnh trạng thái.\n"
            "2. Vào Khách hàng & Dịch vụ để thêm khách hàng và dịch vụ phát sinh.\n"
            "3. Vào Nghiệp vụ lưu trú để tạo booking, nhận phòng, thêm dịch vụ và trả phòng.\n"
            "4. Xem hóa đơn sau khi checkout.\n"
            "5. Vào Thống kê để trình bày doanh thu theo tháng."
        )
        tk.Label(help_box, text=help_text, justify="left", padx=12, pady=10).pack(anchor="w")

    def _build_room_management_tab(self):
        self.room_notebook = ttk.Notebook(self.tab_room_management)
        self.room_notebook.pack(fill="both", expand=True, padx=6, pady=6)

        self.room_type_subtab = ttk.Frame(self.room_notebook)
        self.room_subtab = ttk.Frame(self.room_notebook)

        self.room_notebook.add(self.room_type_subtab, text="Loại phòng")
        self.room_notebook.add(self.room_subtab, text="Phòng & trạng thái")

        self._build_room_types_section(self.room_type_subtab)
        self._build_rooms_section(self.room_subtab)

        if self.user["role"] != "admin":
            self.room_notebook.tab(self.room_type_subtab, state="hidden")

    def _build_customer_service_tab(self):
        self.customer_service_notebook = ttk.Notebook(self.tab_customer_service)
        self.customer_service_notebook.pack(fill="both", expand=True, padx=6, pady=6)

        self.customer_subtab = ttk.Frame(self.customer_service_notebook)
        self.service_subtab = ttk.Frame(self.customer_service_notebook)

        self.customer_service_notebook.add(self.customer_subtab, text="Khách hàng")
        self.customer_service_notebook.add(self.service_subtab, text="Dịch vụ")

        self._build_customers_section(self.customer_subtab)
        self._build_services_section(self.service_subtab)

        if self.user["role"] != "admin":
            self.customer_service_notebook.tab(self.service_subtab, state="hidden")

    def _build_operation_tab(self):
        self.operation_notebook = ttk.Notebook(self.tab_operation)
        self.operation_notebook.pack(fill="both", expand=True, padx=6, pady=6)

        self.booking_subtab = ttk.Frame(self.operation_notebook)
        self.checkout_subtab = ttk.Frame(self.operation_notebook)

        self.operation_notebook.add(self.booking_subtab, text="Đặt & nhận phòng")
        self.operation_notebook.add(self.checkout_subtab, text="Trả phòng & hóa đơn")

        self._build_booking_section(self.booking_subtab)
        self._build_checkout_section(self.checkout_subtab)

    def _build_room_types_section(self, parent):
        form = tk.LabelFrame(parent, text="Thêm loại phòng")
        ...
        delete_box = tk.LabelFrame(parent, text="Xóa loại phòng")
        ...
        self.room_types_tree = self._make_tree(parent, ["ID", "Tên loại", "Giá/đêm", "Sức chứa", "Mô tả"])
        self.room_types_tree.pack(fill="both", expand=True, padx=10, pady=(0, 10))

    def _build_rooms_section(self, parent):
        top_form = tk.LabelFrame(parent, text="Tạo phòng mới")
        top_form.pack(fill="x", padx=10, pady=10)

        self.room_number_entry = ttk.Entry(top_form)
        self.room_type_combo = ttk.Combobox(top_form, state="readonly")
        self.room_status_combo = ttk.Combobox(top_form, state="readonly", values=["Available", "Occupied", "Cleaning", "Maintenance"])
        self.room_note_entry = ttk.Entry(top_form)
        ...
        ttk.Button(top_form, text="Thêm phòng", command=self.add_room).grid(row=1, column=4, padx=8)

        middle = tk.Frame(parent)
        middle.pack(fill="x", padx=10, pady=(0, 10))

        delete_box = tk.LabelFrame(middle, text="Xóa phòng")
        delete_box.pack(side="left", fill="x", expand=True, padx=(0, 5))
        ...
        status_box = tk.LabelFrame(middle, text="Cập nhật trạng thái phòng")
        status_box.pack(side="left", fill="x", expand=True, padx=(5, 0))
        ...
        self.rooms_tree = self._make_tree(parent, ["ID", "Số phòng", "Loại phòng", "Giá/đêm", "Sức chứa", "Trạng thái", "Ghi chú"])
        self.rooms_tree.pack(fill="both", expand=True, padx=10, pady=(0, 10))

    def _build_customers_section(self, parent):
        form = tk.LabelFrame(parent, text="Thêm khách hàng")
        form.pack(fill="x", padx=10, pady=10)
        ...
        delete_box = tk.LabelFrame(parent, text="Xóa khách hàng")
        delete_box.pack(fill="x", padx=10, pady=(0, 10))
        ...
        self.customers_tree = self._make_tree(parent, ["ID", "Họ tên", "SĐT", "CCCD", "Email", "Địa chỉ"])
        self.customers_tree.pack(fill="both", expand=True, padx=10, pady=(0, 10))

    def _build_services_section(self, parent):
        form = tk.LabelFrame(parent, text="Quản lý dịch vụ")
        form.pack(fill="x", padx=10, pady=10)
        ...
        self.services_tree = self._make_tree(parent, ["ID", "Tên dịch vụ", "Đơn giá", "Đơn vị"])
    ...
    def _build_booking_section(self, parent):
        ...
        self.bookings_tree = self._make_tree(outer, ["ID", "Mã booking", "Khách hàng", "Phòng", "Ngày nhận", "Ngày trả", "Trạng thái", "Giá phòng", "Số khách"])
    ...
    def _build_checkout_section(self, parent):
        ...
        self.invoice_text = tk.Text(parent, height=28, font=("Consolas", 11))
